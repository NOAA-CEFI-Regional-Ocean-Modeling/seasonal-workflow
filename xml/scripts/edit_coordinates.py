...
for i in range(1,nens+1):
	...
	ds = xarray.open_dataset(model_output_data / 'extracted'/ args.domain / f'2020-09-e{i:02d}.ocean_month_wrong_coords.nc')
	ds = ds.set_coords(['init', 'member'])
	ds = ds.expand_dims('init')
	ds = ds.transpose(*(['init', 'time'] + [d for d in ds.dims if d not in ['init', 'lead']]))
	ds.to_netcdf(model_output_data / 'extracted'/ args.domain / f'2020-09-e{i:02d}.ocean_month.nc', unlimited_dims='init')
