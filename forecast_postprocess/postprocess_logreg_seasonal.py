...
def main(config: Config, var: str, quantiles: list[float]) -> None:
    ...
    glorys_seasonal_mean = glorys_rg.resample(
        time=SeasonResampler(['JFM', 'AMJ', 'JAS', 'OND'], drop_incomplete=True)
    ).mean()
    qs_file = (
        forecast_output_data
        / 'post_post_processed'
        / f'logreg_quantiles_glorys_seasonal_{var}.nc'
    )
    ...
    if qs_file.is_file():
        ...
        glorys_qs = glorys_seasonal_mean.groupby('time.month').quantile(
            quantiles, dim='time'
        )
    ...
    exceeded = (glorys_seasonal_mean.groupby('time.month') > glorys_qs).astype('int')
    ...
    coefs.to_netcdf(
        forecast_output_data
        / 'post_post_processed'
        / f'logreg_coefs_forecast_seasonal_{var}.nc'
    )
